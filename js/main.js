...
class AllDayEvents extends CalendarDisplay{
    ...
    async update() {
        ...
        promise.then(function() {
            ...
            if (events.length > 0) {
                for (let event of events) {
                    let elTemplate = document.getElementById("message-block");
                    elTemplate.content.querySelectorAll(".message")[0].textContent = event.event.summary;
                    let el = document.importNode(elTemplate.content, true);
                    // el.innerHTML = "(" + new Date(event.event.start.dateTime).toLocaleString() + "): " + (event.calSummary || "No room specified") + " | " + (event.event.summary || "No summary provided.");
                    document.getElementById(self.id).appendChild(el);
                }
    ...
    async update() {
        ...
        promise.then(function() {
            ...
            if (events.length > 0) {
                for (let event of events) {
                    let elTemplate = document.getElementById("calendar-block");
                    elTemplate.content.querySelectorAll(".summary")[0].textContent = (new Date(event.event.start.dateTime).toLocaleString());
                    elTemplate.content.querySelectorAll(".room")[0].textContent = "";           
                    elTemplate.content.querySelectorAll(".time")[0].textContent = event.event.summary;
                    let el = document.importNode(elTemplate.content, true);
                    // el.innerHTML = "(" + new Date(event.event.start.dateTime).toLocaleString() + "): " + (event.calSummary || "No room specified") + " | " + (event.event.summary || "No summary provided.");
                    document.getElementById(self.id).appendChild(el);
                }
